...
def assign_shifts(timetable, duty_timings, troopers, min_hours=5):
    '''
    ...
    for trooper_name in trooper_names:
        if not (troopers[trooper_name]['status'] == 'stay-out' and len([duty for duty in timetable if duty is None]) == 0):
    ...
    for trooper_name, individual_timetable in timetable.items():
        ...
        if doing_first and doing_last:
            ...
            # print('blong2', trooper_name)
            available_shifts[trooper_name].append('afternoon')
        else:
    
            if check_if_all_in_given_shift('morning', individual_timetable) and troopers[trooper_name]['status'] != "stay-out": #Cant assign morning shift for stayout personel
                available_shifts[trooper_name].append('morning')
...
# Manual attempt
def select_shifts(possible_shifts, troopers):
    final_shifts = {
        'morning': set(),
        'afternoon': set(),
        'random': set()
    ...
    # if len(unique_morning) == num_morning:
    #     final_shifts['morning'] = unique_morning
    
    # if len(unique_afternoon) == num_afternoon:
    #     final_shifts['afternoon'] = unique_afternoon
...
def generate_duty_hours(troopers, timetable, duty_timings, shift_dict, roles):
    ...
    # Recursive code to edit - basically you assign longer hours in the order: afternoon --> morning --> random
    # TODO: EDIT THIS CODE
    for i in range(hour_distribution[1][1]):
        if i < len(afternoon_troopers):
            ...
            if index < len(shift_dict['morning']):
                long_duty_trooper = shift_dict['morning'][index]
            else:
                index2 = index - len(shift_dict['morning'])
                long_duty_trooper = shift_dict['random'][index2]
    ...
    # END OF CODE FOR FUNCTIONc-----------------------------------------------
    # pprint.pprint(troopers)




    # If 6 hours, try schedule starting at 12pm finish at 6pm
    # If 5 hours, do the laddering thing (ie the giving breaks accordingly)
    # If 7 hours, do the same laddering (but this one starts at 10am)
    for i in range(len(duty_timings)-1, -1, -1):
        troopers_available = deepcopy(afternoon_troopers)
        timeslot = duty_timings[i]
        occupied_timeslot_roles, troopers_on_duty = get_occupied_roles_for_timeslot(timeslot, duty_timings, timetable)


        troopers_available
    # print(shift_dict, afternoon_troopers)



    
        # if trooper_shift == 'random':
        #     random_shift_troopers.append(trooper_name)
        
        # elif trooper


        # APPROACH: FILL FIRST THEN REMOVE (DEPRECATED)
        # else:
        #     individual_timetable = timetable[trooper_name]
        #     time_range = shift_blocks[trooper_shift]
        #     for duty_index in range(len(individual_timetable)):
        #         duty_timing = duty_timings[duty_index]
        #         duty_name = individual_timetable[duty_index]

        #         if time_range[0] <= duty_timing and time_range[1] >= duty_timing and duty_name == '':
        #             individual_timetable[duty_index] = 'TODO'
    ...
    #         duty_timing = duty_timings[duty_index]
    #         duty_name = individual_timetable[duty_index]
    #         if duty_name is not None: #Dont consider for None --> should not allocate to that shift block
    #             for shift_name, shift_timing in shift_blocks.items(): # Check through all the possible shift blocks for which are suitable
    #                 if (duty_timing <= shift_timing[1] and duty_timing >= shift_timing[0]) and shift_name not in available_shifts[trooper_name]: # If the duty timing lies in the shift block range
                        
    #                     available_shifts[trooper_name].append(shift_name)
    ...
    for role in roles:
        if role == 'sentry':
            ...
            for i in range(len(hour_distribution)):
                ...
                # if i == 0: # if first shift
                #     troopers[selected_trooper]['possible_shifts'] = ['morning', 'random']
                # elif i == len(hour_distribution):
                #     troopers[selected_trooper]['possible_shifts'] = ['afternoon', 'random']
...
def or_tools_role_assignment(troopers, duty_timings, timetable, roles, last_standing_index):
    ...
    for t in all_hours:
        ...
        for p in selected_trooper_indices:
    ...
    for i in range(len(sorted_keys)-1):
        ...
        if p == next_p:
            ...
            if 'TODO' in [current_duty, next_duty] and t not in [0, num_hours-2] and 'sentry' not in [current_duty, next_duty]:
...
def main():
    ...
    # timetable['marc'][6:12] = ['desk', 'in', 'x-ray', 'out', 'PAC', 'PAC']
    ...
    possible_shifts = assign_shifts(timetable, duty_timings, troopers)
    pprint.pprint(possible_shifts)

    allocated_shifts = select_shifts(possible_shifts, troopers)
