...
def role_is_custom(roles_list, value):
    if value in ['', 'TODO'] or value is None:
        return False
    role = find_role(roles_list, value)
...
def compute_total_hours(roles, duty_timings, timetable=None):
    total_hours = 0
    for role in roles:
    ...
    # For custom duties counted in hours
    if timetable is not None:
        for trooper_duties in timetable.values():
            for role_name in trooper_duties:
                role = find_role(roles, role_name)
                if role is not None and role['is_custom'] and role['is_counted_in_hours']:
                    total_hours += 1
...
def find_all_available_shifts(timetable, duty_timings, troopers, roles, shift_blocks, min_hours=5):
    ...
    for trooper_name in trooper_names:
        ...
        # The only caveat is that stayouts can only start after the stayout_earliest_starting_time
...
def determine_shift_blocks(min_hours, max_hours, duty_timings, leeway=2, min_leeway=0, max_leeway=2, mode="static", troopers={}, roles=[], timetable={}, stayout_earliest_starting_time=None):
    '''
    ...
    if mode == 'static':
        ...
        for permutation in permutations:
            ...
            try:
                timetable = or_tools_shift_scheduling(troopers, duty_timings, timetable_copy, roles, shift_blocks, stayout_earliest_starting_time)
                solution_found = True
                break
...
def select_shifts(possible_shifts, troopers, shift_distribution, stayout_afternoon_shift_probability=0.9):
    ...
    for trooper_name, trooper_shifts in possible_shifts.items():
        for shift in trooper_shifts:
            # In most cases, stayout troopers are given afternoon shift 
            # since this is more convenient to them. However, they can 
            # also take on random shifts instead. This code determines if they should 
            # be confirm allocated an afternoon shift or just based on luck later randomly allocated
            if 'random' not in trooper_shifts and \
                random.random() < stayout_afternoon_shift_probability:
                print('plomng test')
...
def add_autogenerated_hours(troopers, timetable, roles):
    '''
    When the duty hours are generated, the 'assigned_hours' originally referred
    to the number of hours that had to be autogenerated by OR Tools functions.
    However, after adding custom roles (that are counted in hours), these are 
    hours that are not autogenerated but added by the user alone. Hence, 
    another parameter is needed, 'autogenerated_hours' for OR tools to know
    how many duties to allocate to each person.
    '''
    for trooper_name in troopers:
        trooper_dict = troopers[trooper_name]
        trooper_duty = timetable[trooper_name]
        
        custom_hours = 0
        for role_name in trooper_duty:
            role = find_role(roles, role_name)
            if role is not None and role['is_custom'] and role['is_counted_in_hours']:
                custom_hours += 1
        
        trooper_dict['autogenerated_hours'] = trooper_dict['assigned_hours'] - custom_hours
...
def generate_duty_hours(troopers, duty_timings, shift_dict_to_copy, roles, timetable):
    ...
    total_hours = compute_total_hours(roles, duty_timings, timetable=timetable)
...
def or_tools_shift_scheduling(troopers, duty_timings, timetable, roles, shift_blocks, stayout_earliest_starting_time):
    # FInd the actual number of hours needed to generate 
    # (by subtracting the number of hours that are custom duties, counted in hours)
    troopers = add_autogenerated_hours(troopers, timetable, roles)
    ...
    for p in all_troopers:
        ...
        for t in all_hours:
            ...
            # Block off earlier timeslots than the stayout_earliest_starting_time
            trooper_dict = troopers[trooper]
            if trooper_dict['status'] == 'stay-out' and timing < stayout_earliest_starting_time:
                model.add(duties[(p, t)] == 0)

            # Assign no slot for None values in timetable AND custom roles (basically like a None block)
            ...
            if timetable[trooper][t] is None or role_is_custom(roles, timetable[trooper][t]):
    ...
    for p in all_troopers:
        ...
        autogenerated_hours = troopers[trooper_name]['autogenerated_hours']
        # print(assigned_hours)

        model.add(sum(duties[(p, t)] for t in all_hours) == autogenerated_hours)
...
def or_tools_role_assignment(troopers, duty_timings, timetable, roles, last_standing_index):
    ...
    for t in all_hours:
        ...
        for p in all_troopers:
            ...
            # NOTE: Change from the below code to rn since all custom roles are not autogenerated
            # if current_duty is not None and current_duty != '' and not role_is_custom_and_not_counted(roles, current_duty):
            if current_duty is not None and current_duty != '' and not role_is_custom(roles, current_duty):
